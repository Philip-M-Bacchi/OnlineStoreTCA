    ...
    static let live = Self(
        fetchProducts: {
            let (data, _) = try await URLSession.shared
                .data(from: URL(staticString: "https://fakestoreapi.com/products"))
            let products = try JSONDecoder().decode([Product].self, from: data)
            return products
        },
        sendOrder: { cartItems in
            let payload = try JSONEncoder().encode(cartItems)
            var urlRequest = URLRequest(url: URL(staticString: "https://fakestoreapi.com/carts"))
        ...
        fetchUserProfile: {
            let (data, _) = try await URLSession.shared
                .data(from: URL(staticString: "https://fakestoreapi.com/users/1"))
